...
- `/vllm_deployment` — Qwen 3.5 27B inference on GPU Cloud Run (scale-to-zero)
...
Web / Mobile → REST API → FastAPI (Cloud Run) → Supabase (PostgreSQL)
                                              ↘ vLLM Qwen 3.5 27B (Cloud Run GPU)
```
...
- vLLM endpoint uses OpenAI-compatible API (model: `qwen3.5-27b`)
