...
When configuring the Google Cloud Run deployment for the vLLM Qwen2.5-VL-7B-Instruct container:
- You MUST explicitly set `min-instances=0`.
- The GPU container must scale to zero when idle to prevent excessive cloud billing.
- Expected cold start time: ~15-30 seconds for the 7B model (vs 60-90s for the former 27B model).
