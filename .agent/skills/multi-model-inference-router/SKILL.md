...
User picks model
      ...
      │
      └─ "On-Device (3.8B)" ► llama.rn local ──────────► S25 Adreno GPU
                               (no network)                Phi-3.5-mini
```
...
```jsx
const MODELS = [
  { id: 'cloud',   label: 'Cloud',    sub: '9B',   icon: 'cloud-outline' },
  { id: 'home_pc', label: 'Home PC',  sub: '7B',   icon: 'desktop-outline' },
  { id: 'device',  label: 'On-Device',sub: '3.8B', icon: 'phone-portrait-outline' },
];
...
- **Do NOT bundle the GGUF file in the APK** — it is 2.4GB. Always download on first use.
