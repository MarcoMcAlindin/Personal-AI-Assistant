...
* [cite_start]**Three-Tier Inference Architecture (via vLLM / Ollama / on-device):** VibeOS routes AI requests across three backends based on `model_target`. Cloud: `Qwen/unsloth/Qwen3.5-35B-A3B-GGUF (Q4_K_M)` on Cloud Run L4 GPU (europe-west1). Home PC: `DavidAU/Qwen3.5-9B-Claude-4.6-HighIQ-INSTRUCT` via Tailscale/Ollama on RTX 4070 Ti. On-Device: `DavidAU/Qwen3.5-2B-GPT-5.1-HighIQ-INSTRUCT` running locally on mobile. The cloud tier scales to zero when idle and activates instantly for chat inference or health analysis. [cite: 89, 90]
* [cite_start]**GitHub Actions Automation:** Every day at exactly 8:00 AM GMT, a scheduled workflow executes a Python script that pulls whatever health data was most recently synced to Supabase, sends it to the cloud model (`unsloth/Qwen3.5-35B-A3B-GGUF (Q4_K_M)`), and writes the personalized analysis back to the database. [cite: 91]
